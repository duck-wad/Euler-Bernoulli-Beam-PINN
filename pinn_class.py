...
class BeamPINN(nn.Module):
    def __init__(self, num_input, num_output, num_neurons, num_layers):
        ...
        self.input_layer = nn.Sequential(
            nn.Linear(num_input, num_neurons), activation()
        )
        # define the hidden layer as sequence of num_layers
        self.hidden_layer = nn.Sequential(
            *[
                nn.Sequential(nn.Linear(num_neurons, num_neurons), activation())
    ...
    def PDE_loss(self, Y_domain, X_domain, E, I):
        ...
        dy = torch.autograd.grad(
            Y_domain,
            X_domain,
            torch.ones_like(Y_domain),
            retain_graph=True,
            create_graph=True,
        )[0]
        d2y = torch.autograd.grad(
            dy, X_domain, torch.ones_like(dy), retain_graph=True, create_graph=True
        )[0]
        d3y = torch.autograd.grad(
            d2y, X_domain, torch.ones_like(d2y), retain_graph=True, create_graph=True
        )[0]
        d4y = torch.autograd.grad(
            d3y, X_domain, torch.ones_like(d3y), create_graph=True
        )[0]
        ...
        f = E * I * d4y_dx4
        f_hat = X_domain[:, 1].unsqueeze(
            1
        )  # THIS MAY NEED TO CHANGE LATER FOR NON-CONSTANT DISTRIBUTED LOADS
        return self.loss_function(f, f_hat)

    def boundary_loss(self, Y_bc, X_bc, E, I):
        # handle when Y_bc is empty
        if Y_bc.numel() == 0:
            return torch.tensor(0.0, device=Y_bc.device), torch.tensor(
                0.0, device=Y_bc.device
            )
        # for simply supported beam, y=0 and d2ydx2=0
        dy = torch.autograd.grad(
            Y_bc, X_bc, torch.ones_like(Y_bc), retain_graph=True, create_graph=True
        )[0]
        d2y = torch.autograd.grad(dy, X_bc, torch.ones_like(dy), create_graph=True)[0]

        d2y_dx2 = d2y[:, 0].unsqueeze(1)
        return (
            self.loss_function(Y_bc, torch.zeros_like(Y_bc)),
            self.loss_function((E * I * d2y_dx2), torch.zeros_like(d2y_dx2)),
        )
    ...
    def slope_data_loss(self, Y_pred, X, DYDX_true):
        ...
        dy = torch.autograd.grad(
            Y_pred, X, torch.ones_like(Y_pred), retain_graph=True, create_graph=True
        )[0]
    ...
    def return_derivatives(self, Y, X):
        ...
        d2y = torch.autograd.grad(
            dy, X, torch.ones_like(dy), retain_graph=True, create_graph=True
        )[0]
        d3y = torch.autograd.grad(
            d2y, X, torch.ones_like(d2y), retain_graph=True, create_graph=True
        )[0]
